            ...
            if (list.length>0){
                for (var i = 0; i < list.length; i++) {
                    ...
                    var html='<div class="card mb-4">'
                        ...
                        +'<a class="product-thumb" href="http://localhost:8080/products/#{pid}">'
                        +'<div class="card-body">'
                        +'<div class="d-flex"><a class="pr-4 hidden-xs-down search-products" href="http://localhost:8080/products/#{pid}">'
                        +'<img src="..#{image}" alt="Product"></a>'
                        +'<div>'
                        +'<h5><a class="navi-link" href="http://localhost:8080/products/#{pid}">#{name}</a></h5>'
                        +'<h6>￥#{price}</h6>'
                        +'<p>#{information}</p>'
                        +'</div>'
                        +'</div>'
